...
async def start(message: types.Message):
    ...
    have_bought_course = False

    if not database.user_exist(user_id):
        database.insert_user(user_id, user_full_name)
        #database.insert_promo_code(promo, 50, user_id)
    else:
        have_bought_course = get_user_by_id(message.from_user.id)[5]


    await message.answer("Добро пожаловать! \nВыберите одну из функций:",  reply_markup=main_menu_button(have_bought_course))
